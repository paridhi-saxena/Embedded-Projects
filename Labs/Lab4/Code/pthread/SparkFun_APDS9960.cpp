...
bool SparkFun_APDS9960::init()
{
   ...
   // Wire.begin();
    ...
    if(file < 0) {
	  std::cout <<"error opening device file\n"<<std::endl;
    }  
...
int SparkFun_APDS9960::readGesture()
{
	...
	usleep(FIFO_PAUSE_TIME*1000000);
...
bool SparkFun_APDS9960::wireWriteDataByte(uint8_t reg, uint8_t val)
{
    ...
    if (ioctl(file, I2C_SLAVE, addr) < 0) {
	    std::cout<<"error in i2c addr"<<std::endl;
    }
...
bool SparkFun_APDS9960::wireReadDataByte(uint8_t reg, uint8_t &val) {
    ...
    if (ioctl(file, I2C_SLAVE, addr) < 0) {
    	std::cout<<"error in i2c addr read"<<std::endl;
    }
                                        ...
                                        unsigned int len)
{
    ...
    while (Wire.available()) {
        ...
        std::cout<<"start read block"<<std::cout;
	int addr = 0x39; /* The I2C address */
  	if (ioctl(file, I2C_SLAVE, addr) < 0) {
	...
	buf[0] = reg;
	for(i = 0; i < len; i++) {
		val[i] = i2c_smbus_read_byte_data(file, reg);
	/*if (read(file, buf, 1) != 1) {
		std::cout<<"block read failed"<<std::endl;
  	} else {
